...
| F04 GRN vs bill | Resolved 27 Sep (GRNI model): receipt posts Dr Inventory / Cr 2150 GRNI only; matched bill posts Dr GRNI / Dr Input VAT / Cr AP; line-level `supplier_bill_grn_matches`; over-receipt, supplier, order-state, scope and stale-duplicate guards; historical rows reported by `finance:grn-bill-overlap`, not rewritten | see `docs/f04-grni-sprint-2026-09-27.md` | `GrnBillMatchingTest` (14), FIN-08 PASS |
